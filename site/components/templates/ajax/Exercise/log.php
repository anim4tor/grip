<?php
	...
	$exercise ??= null;
	$exercise = $kirby->controller('get_log_exercise', [ 'kirby' => $kirby, 'id' => $exercise, 'workout' => $workout['id'] ]);
	extract($exercise);
?>
<form id="update_exercise" action="form/update_exercise" method="post">
	...
	<toolbar class="sticky inset__top-stretch inner-r__1 inner-t__2 inner-b__1 flex align__center justify__space-between gap__05 z__1 bg__inherit">
		<a modal-reveal modal-open="prev" href="modal/workout/log/workout=<?= $workout['id'] ?>" class="button circle"><icon><?= svg('public/assets/images/ui/ui_arrow-left.svg') ?></icon></a>
