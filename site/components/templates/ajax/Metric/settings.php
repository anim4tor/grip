...
<form action="form/add_exercise" method="post" modal-back="modal/workout/index/workout=<?= $id ?>">
	...
	<toolbar class="sticky inset__top-stretch inner-t__2 inner-b__1 inner-r__1 flex align__center justify__space-between gap__2 z__2 bg__inherit">
		<a modal-reveal modal-open="prev" href="modal/metric/log/metric=<?= $id ?>" class="button circle"><icon><?= svg('public/assets/images/ui/ui_arrow-left.svg') ?></icon></a>
		<nav class="button__group">
			<a open-dialog href="modal/dialog/delete_metric/metric=<?= $id ?>" modal-reveal class="button circle bg__red color__invert"><icon><?= svg('public/assets/images/ui/ui_delete.svg') ?></icon></a>
		</nav>
	</toolbar>
	<header class="">
