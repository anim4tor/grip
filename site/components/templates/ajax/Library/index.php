<?php
	...
	$exercises ??= [];
?>
<form action="form/select_exercise" method="post" modal-back="modal/workout/index/workout=<?= $id ?>">
	...
	<toolbar class="sticky inset__top-stretch inner-t__2 inner-b__1 inner-r__1 flex align__center justify__space-between gap__2 z__2 bg__inherit">
		...
		<nav class="button__group">
			<a modal-reveal modal-open="next" href="modal/exercise/add/workout=<?= $workout['id'] ?>" class="button circle bg__invert/20"><icon><?= svg('public/assets/images/ui/ui_add.svg') ?></icon></a>
		...
		<div class="inner-x__1 inner-b__2 flex align__center justify__space-between gap__05">
			<div class="flex gap__1 align__center">
				...
				<div class="grid gap__05 align__center inner-t__04">
					<h1>Library</h1>
					<p class="font__size__lg"><span class="op__6"><?= count(collection('Exercises')) ?> exercises related to</span> Upper body</p>
				</div>
			</div>
	...
	<section>
		<list class="grid inner-y__1">
			<?php foreach (collection('Exercises') as $key => $exercise) : ?>
