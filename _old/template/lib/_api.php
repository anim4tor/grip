<?php
	...
	global $root; 
	$root = ($_SERVER['SERVER_NAME']==='grip') ? '' : '/projects/myWorkoutApp/';
	// $root = '/';
	...
	*/
	...
	$_GLOBAL['db'] = json_decode(file_get_contents('template/lib/_db.json'), true)['dist'][0];
	...
	*/
	...
	function load_week($day = "Today") {
		...
		$todayDate = date('Y-m-d', strtotime($day));
		...
		for ($i = 0; $i < 7; $i++) {
		    $week[$i]['date'] = date('Y-m-d', strtotime($timestamp));
		    $week[$i]['daynum'] = date('N', strtotime($timestamp));
		    $week[$i]['name'] = date('l', strtotime($timestamp));
